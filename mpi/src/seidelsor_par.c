	...
	*/
	...
	gettimeofday(&totals,NULL);
 	...
 	//----Computational core----//   
		...
		// Process 0 must unblock here
		// Everyone who has north and west neighbor must block here 
		// because they are computed here  
		if (len != 0) {
    ...
    if (rank==0) {
		printf("GaussSeidel X %d Y %d Px %d Py %d Iter %d CommunicationTime %lf ComputationTime %lf TotalTime %lf midpoint %lf\n",
