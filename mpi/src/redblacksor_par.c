	...
	*/
	...
	gettimeofday(&totals,NULL);
 	...
 	//----Computational core----//   
		...
		if (north >= 0) {
			MPI_Isend(&u_previous[1][0], 1, row, north, t, MPI_COMM_WORLD, &request[len]);
			len++;
                        MPI_Irecv(&u_previous[0][0], 1, row, north, t, MPI_COMM_WORLD, &request[len]);
			len++;
		}		
		if (south >= 0) {
			MPI_Isend(&u_previous[local[0]][0], 1, row, south, t, MPI_COMM_WORLD, &request[len]);
			len++;
			MPI_Irecv(&u_previous[local[0]+1][0], 1, row, south, t, MPI_COMM_WORLD, &request[len]);
			len++;
		}
		if (east >= 0) {
			MPI_Isend(&u_previous[0][local[1]], 1, column, east, t, MPI_COMM_WORLD, &request[len]);
			len++;
			MPI_Irecv(&u_previous[0][local[1]+1], 1, column, east, t, MPI_COMM_WORLD, &request[len]);
			len++;
		}
		if (west >= 0) {
			MPI_Isend(&u_previous[0][1], 1, column, west, t, MPI_COMM_WORLD, &request[len]);
			len++;
			MPI_Irecv(&u_previous[0][0], 1, column, west, t, MPI_COMM_WORLD, &request[len]);
			len++;
		}
		...
		if (north >= 0) {
			MPI_Isend(&u_current[1][0], 1, row, north, t, MPI_COMM_WORLD, &request[len]);
			len++;
                        MPI_Irecv(&u_current[0][0], 1, row, north, t, MPI_COMM_WORLD, &request[len]);
			len++;
		}		
		if (south >= 0) {
			MPI_Isend(&u_current[local[0]][0], 1, row, south, t, MPI_COMM_WORLD, &request[len]);
			len++;
			MPI_Irecv(&u_current[local[0]+1][0], 1, row, south, t, MPI_COMM_WORLD, &request[len]);
			len++;
		}
		if (east >= 0) {
			MPI_Isend(&u_current[0][local[1]], 1, column, east, t, MPI_COMM_WORLD, &request[len]);
			len++;
			MPI_Irecv(&u_current[0][local[1]+1], 1, column, east, t, MPI_COMM_WORLD, &request[len]);
			len++;
		}
		if (west >= 0) {
			MPI_Isend(&u_current[0][1], 1, column, west, t, MPI_COMM_WORLD, &request[len]);
			len++;
			MPI_Irecv(&u_current[0][0], 1, column, west, t, MPI_COMM_WORLD, &request[len]);
			len++;
		}
		MPI_Waitall(len, request, status);
		gettimeofday(&commf,NULL);
    ...
    if (rank==0) {
		printf("RedBlackSOR X %d Y %d Px %d Py %d Iter %d CommunicationTime %lf ComputationTime %lf TotalTime %lf midpoint %lf\n",
